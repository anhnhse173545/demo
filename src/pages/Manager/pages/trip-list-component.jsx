...
export function TripListComponent() {
  ...
  const handleUpdateStatus = useCallback(async (tripId, newStatus) => {
    ...
    try {
      await api.put(
        `/update/${tripId}`,
        { status: newStatus === 'no-status' ? '' : newStatus }
      )
      await fetchTrips()
    } catch (error) {
      handleError(error)
  ...
  const filteredAndSortedTrips = useMemo(() => {
    return trips
      .filter(trip => {
        ...
        if (filterStatus === 'Others') return !['Seasonal', 'Completed', 'Pending', 'Confirmed', 'Canceled'].includes(trip.status);
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <Select
                    ...
                    <SelectContent>
                      {['Seasonal', 'Completed', 'Pending', 'Confirmed', 'Canceled', 'no-status'].map((status) => (
